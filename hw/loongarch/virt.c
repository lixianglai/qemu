                                ...
                                DeviceState *dev, Error **errp)
{
    ...
    if (lvms->acpi_ged) {
        ...
        /* connect ipi irq to cpu irq */
        qdev_connect_gpio_out(lvms->ipi, CPU(dev)->cpu_index,
                              qdev_get_gpio_in(dev, IRQ_IPI));
        env->ipistate = lvms->ipi;

        for (pin = 0; pin < LS3A_INTC_IP; pin++) {
            qdev_connect_gpio_out(lvms->extioi, (cpu->phy_id * 8 + pin),
...
static void virt_get_cpu_topo_from_index(MachineState *ms,
                                         LoongArchCPUTopo *topo, int cpu_index)
{
    topo->socket_id  = cpu_index / (ms->smp.cores * ms->smp.threads);
    topo->core_id = cpu_index / ms->smp.threads % ms->smp.cores;
    topo->thread_id = cpu_index % ms->smp.threads;
}
